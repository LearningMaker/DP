...
class FinalBlock(nn.Module):
    ...
    def forward(self, x, lstm_x):
        # lstm_x = self.lstm_pointwise(lstm_x)
        # lstm_x = F.interpolate(lstm_x, size=x.size(2), mode="linear", align_corners=True)
        #
        # out = torch.cat([x, lstm_x], dim=1)
        # out = self.pointwise(out)
